...
### 🪄⭐统一网关（多通道共享上下文）
一个进程即可同时支持多个接口（终端、Slack、Gradio 实时界面、WhatsApp、Maid），共享上下文：

```bash
aeiva-gateway --config configs/agent_config.yaml --verbose
```

* **开关接口**在 `configs/agent_config.yaml` 中配置：
	- `terminal_config.enabled`
	- `slack_config.enabled`
	- `realtime_config.enabled`（Gradio 实时界面）
	- `whatsapp_config.enabled`
	- `maid_config.enabled`
* **Gradio 实时界面**开启后访问 `http://127.0.0.1:7860`。
* **共享上下文**：默认所有启用的通道共用同一代理状态。
* 如需**独立进程/上下文**，请使用下面的单独命令。
