...
from aeiva.util.constants import MODALITY_NOT_EXIST_ID
...
from aeiva.util.constants import OPENAI_CLIP_MEAN, OPENAI_CLIP_STD
try:
...
def generate_audio_id(example: dict, id_generator: IDGenerator = None):
    data = example['audio']
    if data is None:
        example['audio_id'] = MODALITY_NOT_EXIST_ID
    else:
        example['audio_id'] = id_generator.get_id(data)
    return example


def generate_image_id(example: dict, id_generator: IDGenerator = None):
    data = example['image']
    if data is None:
        example['image_id'] = MODALITY_NOT_EXIST_ID
    else:
        example['image_id'] = id_generator.get_id(data)
    return example


def generate_video_id(example: dict, id_generator: IDGenerator = None) -> dict:
    data = example['video']
    if data is None:
        example['video_id'] = MODALITY_NOT_EXIST_ID
    else:
        example['video_id'] = id_generator.get_id(data)
    return example
