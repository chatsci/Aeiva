...
Reference: https://github.com/lyuchenyang/Macaw-LLM/tree/main

Author: Bang Liu
Date: 2023/06/19
"""
# Note: before running this script, you need to do the following:
# Stage 1:
# 0. Create a folder named "avsd/" and a folder named "coco/" in the current folder.
# 1. Download the COCO image dataset (2014 Train images [83K/13GB], 2014 Val images [41K/6GB]) from: https://cocodataset.org/#download, unzip and put images into coco/train2014/, coco/val2014.
# 2. Download the Macaw dataset: https://github.com/lyuchenyang/Macaw-LLM/blob/main/data/generated_examples_coco.json, https://github.com/lyuchenyang/Macaw-LLM/blob/main/data/generated_examples_avsd.json
# 3. Download the Charades video dataset (Data (scaled to 480p, 13 GB)) from: https://prior.allenai.org/projects/charades, unzip (Charades_v1_480/), move all the videos from "Charades_v1_480/" to "./avsd/videos/".
# Stage 2:
# 0. Create a folder named "vqa/" in the current folder.
...
# This script is adapted from: https://github.com/lyuchenyang/Macaw-LLM/blob/main/preprocess_data.py
import os
import sys
import numpy as np
import pandas as pd
...
def pretty_save_json(file_path, output_path):
...
def extract_audio_from_video(videos_folder_path, output_folder_path):
...
def sample_frames_from_video(videos_folder_path, output_folder_path, frames_per_video=120):
...
def _get_image_and_video_names_for_vqa_and_avsd_datasets(vqadataset_paths, avsddataset_paths, save_path):
    ...
    save_to_json(visual_data_names, save_path)
...
def _get_image_and_video_names_from_macaw_generated_coco_and_avsd_datasets(macaw_coco_dataset_path, macaw_avsd_dataset_path, output_path):
    ...
    save_to_json(all_names, output_path)
...
if __name__ == '__main__':
    ...
    # sample_frames_from_video(videos_folder_path, frames_folder_path)
    # extract_audio_from_video(videos_folder_path, audios_folder_path)
    ...
    # pretty_save_json(join(vqa_folder_path, 'v2_mscoco_train2014_annotations.json'), join(vqa_folder_path, 'v2_mscoco_train2014_annotations.pretty.json'))
    # pretty_save_json(join(vqa_folder_path, 'v2_mscoco_val2014_annotations.json'), join(vqa_folder_path, 'v2_mscoco_val2014_annotations.pretty.json'))
    # pretty_save_json(join(vqa_folder_path, 'v2_OpenEnded_mscoco_train2014_questions.json'), join(vqa_folder_path, 'v2_OpenEnded_mscoco_train2014_questions.pretty.json'))
    # pretty_save_json(join(vqa_folder_path, 'v2_OpenEnded_mscoco_val2014_questions.json'), join(vqa_folder_path, 'v2_OpenEnded_mscoco_val2014_questions.pretty.json'))

    # pretty_save_json(join(avsd_folder_path, 'avsd_train.json'), join(avsd_folder_path, 'avsd_train.pretty.json'))
    # pretty_save_json(join(avsd_folder_path, 'avsd_val.json'), join(avsd_folder_path, 'avsd_val.pretty.json'))
    # pretty_save_json(join(avsd_folder_path, 'train_options.json'), join(avsd_folder_path, 'train_options.pretty.json'))
    # pretty_save_json(join(avsd_folder_path, 'val_options.json'), join(avsd_folder_path, 'val_options.pretty.json'))
